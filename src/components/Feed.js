...
function Feed({ posts, handleAddPost }) {
  ...
  return (
    <div className="bg-yellow-100 min-h-screen flex flex-col pl-72 pt-10">
      ...
      <form onSubmit={(e) => handleAddPost(newPost, e)} className="flex flex-col mr-32 items-start">
          <textarea 
            name="content"
      ...
      {sortedPosts.map(post => {
        return (
          <div key={post.content} className="flex flex-col mr-20 rounded-md ring-2 ring-gray-300 my-2 bg-amber-200 group">
            <div className="flex flex-row">
              <img src="https://img.icons8.com/external-bearicons-glyph-bearicons/64/000000/external-User-essential-collection-bearicons-glyph-bearicons.png"
                              ...
                              alt="Avatar"/>
              <p className="max-w-fit my-1 p-1 rounded-lg font-semibold">{post.user.username}</p>
            </div>
            <p className="px-3">{post.content}</p>
            <button className="max-w-fit px-1 rounded-md ml-2 text-white ring-2 ring-gray-300 my-2 bg-gray-400 hover:bg-gray-600 hidden group-hover:block">Like</button>
          </div>
        )
      })}
